...
var model = {
  ...
  ,stock: function(id){
    ...
    return new Promise(function(resolve, reject){
      ...
      var sqry = "SELECT P.id, P.name, PC.upc, C.color, C.name AS des \
          FROM productos P \
  ...
  ,search: function(searchFld){
    return new Promise(function(resolve, reject){
      ...
      WHERE P.name LIKE '%s' OR P.upc = '%s'\
      UNION \
      SELECT P.id, P.name, PC.upc, P.price, P.tipo, P.origen, C.name as color \
      FROM productos P \
      JOIN producto_colores PC on P.id = PC.p_id \
      JOIN colores C ON PC.option_id = C.id \
      WHERE PC.upc = '%s'";
  ...
  ,saveColor: function(item){
    return new Promise(function(resolve, reject){
      // var qry = util.format('INSERT INTO combinations (p_id, option_id, upc ) VALUES (%s, %s, "%s")',
      var qry = util.format('INSERT INTO producto_colores (p_id, option_id, upc ) VALUES (%s, %s, "%s")',
        item.p_id, item.option_id, item.upc || "");
      console.log('qry', qry);
      db.all(qry, function(err, rows) {
          console.log('rows', rows);
          resolve(rows);
